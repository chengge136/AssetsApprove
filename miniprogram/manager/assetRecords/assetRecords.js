...
Page({
  ...
  onLoad: function (options) {
    ...
    var assetsStorage = wx.getStorageSync('assetsStorage');
    var reloadTime = wx.getStorageSync('reloadTime');
    this.setData({
      assetsCount:assetsStorage,
  ...
  reload:function(){
    ...
    wx.cloud.callFunction({
      ...
      for(var j=0;j< records.length;j++){
        console.log('record',records[j]);
        //分割物资
        var items = records[j].itemsinfo.split(";");
        for (var i = 0; i < items.length-1; i++) {
          var item = items[i].split("-");
          var exist = assetsStorage.find(function (ele) {
            return ele.name === item[0]
          })
  ...
  getExcel:function(){
    ...
    var that = this;
      if (!that.data.fileUrl == ''){
        ...
        wx.showModal({
          title: '导出数据',
          content: '确定导出[ ' + that.data.currentYear + ' ]年的物资申领报表吗？',
          success(res) {
            if (res.confirm) {
              wx.showLoading({
                title: '正在导出',
              ...
              wx.cloud.callFunction({
                ...
                for(var i=0;i<assetsdata.length;i++){
                  ...
                  assetsdata[i].assettype = app.returnHanAssetType(assetsdata[i].assettype);
